...
import { mintCampusEnquiryToken } from "@/lib/campus-enquiry-tokens";
...
export async function GET(request: Request) {
  ...
  const rl = rateLimitMemory(`campus-enquiry-token:${ip}`, 40, 60 * 60 * 1000);
  if (!rl.ok) {
  ...
  const token = mintCampusEnquiryToken();
